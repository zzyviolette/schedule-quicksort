...
void benchmark(int type, int nthreads, double delay){
    FILE *fw,*ft;
    if(type ==1 ){
       fw = fopen("LIFO.values", "a+");
    }else{
       fw = fopen("Stealing.values", "a+");
    }
    ft = fopen("title.value", "a+");
    fprintf(ft, "Quicksort avec %d coeurs \n",sched_default_threads());
    fclose(ft);
    fprintf(fw, "%d\t",nthreads);
    fprintf(fw, "%lf\t",delay);
    fprintf(fw, "%d/%d\n", get_reussi(), get_reussi()+get_echec());
    fclose(fw);
}
...
main(int argc, char **argv)
{
    ...
    if(strcmp("./worksharing",argv[0])==0){
        type = 1;//LIFO
      }else{
        type = 2;//Stealing
    }
    if(!serial){//non serial
      benchmark(type, nthreads, delay);
    }
    for(int i = 0; i < n - 1; i++) {
      assert(a[i] <= a[i + 1]);
    }
